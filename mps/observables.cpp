...
T psiHphi (MPS<T>& psi, MPO<T>& H, MPS<T>& phi) {
  ...
  dtensor_index l1, l2, l3, r1, r2, r3;
  l1 = dtensor_index(1,"ID"+to_string(psi._id)+"Link0",Link,1);
  l2 = dtensor_index(1,"ID"+to_string(H._id)+"Link0",Link);
  l3 = dtensor_index(1,"ID"+to_string(phi._id)+"Link0",Link);

  r1 = dtensor_index(1,"ID"+to_string(psi._id)+"Link"+to_string(psi.length),Link,1);
  r2 = dtensor_index(1,"ID"+to_string(H._id)+"Link"+to_string(psi.length),Link);
  r3 = dtensor_index(1,"ID"+to_string(phi._id)+"Link"+to_string(phi.length),Link);
  for (size_t i = 0; i < psi.length; i++) {
    t1 = psi.A[i]; t1.dag(); t1.conj(); t1.prime();
    t2 = H.A[i];
    t3 = phi.A[i];
    if(i==0){
      dtensor<T> left_ends({l1,l2,l3});
      left_ends.setOne();
      t4 = std::move(left_ends*t1);
      t5 = std::move(t4*t2);
      acc= std::move(t5*t3);
    }else if(i==psi.length-1){
      dtensor<T> right_ends({r1,r2,r3});
      right_ends.setOne();
      t4 = std::move(acc*t1);
      t5 = std::move(t4*t2);
      acc= std::move(t5*t3);
...
T psiHKphi(MPS<T>& psi, MPO<T>& H, MPO<T>& K, MPS<T>& phi){
  ...
  dtensor_index l1, l2, l3, l4 , r1, r2, r3, r4;
  l1 = dtensor_index(1,"ID"+to_string(psi._id)+"Link0",Link,2);
  l2 = dtensor_index(1,"ID"+to_string(H._id)+"Link0",Link,1);
  l3 = dtensor_index(1,"ID"+to_string(K._id)+"Link0",Link);
  l4 = dtensor_index(1,"ID"+to_string(phi._id)+"Link0",Link);

  r1 = dtensor_index(1,"ID"+to_string(psi._id)+"Link"+to_string(psi.length),Link,2);
  r2 = dtensor_index(1,"ID"+to_string(H._id)+"Link"+to_string(psi.length),Link,1);
  r3 = dtensor_index(1,"ID"+to_string(K._id)+"Link"+to_string(phi.length),Link);
  r4 = dtensor_index(1,"ID"+to_string(phi._id)+"Link"+to_string(phi.length),Link);
  for (size_t i = 0; i < psi.length; i++) {
    ...
    t4 = phi.A[i];
    if(i==0){
      dtensor<T> left_ends({l1,l2,l3,l4});
      left_ends.setOne();
      ...
      dtensor<T> right_ends({r1,r2,r3,r4});
      right_ends.setOne();
      t5 = std::move(acc*t1);
      t6 = std::move(t5*t2);
      t7 = std::move(t6*t3);
...
T psiphi (MPS<T>& psi, MPS<T>& phi) {
  ...
  dtensor_index l1, l2, r1, r2;
  //l1=prime(psi.A[0].idx_set[0]); l2=(phi.A[0].idx_set[0]);
  //r1=prime(psi.A[psi.length-1].idx_set[0]); r2=(phi.A[phi.length-1].idx_set[0]);
  l1 = dtensor_index(1,"ID"+to_string(psi._id)+"Link0",Link,1);
  l2 = dtensor_index(1,"ID"+to_string(phi._id)+"Link0",Link);
  r1 = dtensor_index(1,"ID"+to_string(psi._id)+"Link"+to_string(psi.length),Link,1);
  r2 = dtensor_index(1,"ID"+to_string(phi._id)+"Link"+to_string(phi.length),Link);
  ...
  for (size_t i = 0; i < psi.length; i++) {
    ...
    if(i==0){
      ...
      dtensor<T> left_ends({l1, l2});
      left_ends.setOne();
      t3 = std::move(left_ends*t1);
      acc=std::move(t3*t2);
    }else if(i==psi.length-1){
      dtensor<T> right_ends({r1,r2});
      right_ends.setOne();
      t3 = std::move(t1*acc);
      acc= std::move(t3*t2);
      res= acc.contract(right_ends);
