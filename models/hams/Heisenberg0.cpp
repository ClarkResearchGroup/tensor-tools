  ...
  for (size_t k = 0; k < 2; k++) {
    for (size_t b = 0; b < 2; b++) {
      ...
      if(H.A[site].block_id_by_qn_str.count(qn_str) > 0){
        ...
        unsigned s0 = H.A[site].block[block_pos].stride(0);
        unsigned s1 = H.A[site].block[block_pos].stride(1);
        unsigned s2 = H.A[site].block[block_pos].stride(2);
        unsigned s3 = H.A[site].block[block_pos].stride(3);
        if(site==0){
          assert(s3*c < H.A[site].block_size[block_pos]);
          H.A[site].block[block_pos].data()[s3*c] = val * (*_s).d_bra_op_ket(b, op, k);
        }else{
          assert(s0*r + s3*c < H.A[site].block_size[block_pos]);
          H.A[site].block[block_pos].data()[s0*r + s3*c] = val * (*_s).d_bra_op_ket(b, op, k);
        }
      }
    }
  }
